...
export default function CreateModulePage() {
  ...
  async function uploadVideo(file: File): Promise<string | null> {
    try {
      ...
      const fileName = `module-video-${Date.now()}-${Math.random()
        .toString(36)
        .substring(2)}.${fileExt}`;
  ...
  async function uploadFiles(files: File[]) {
    ...
    try {
      for (const file of files) {
        // Generate unique filename
        const fileExt = file.name.split(".").pop();
        const fileName = `module-file-${Date.now()}-${Math.random()
          .toString(36)
          .substring(2)}.${fileExt}`;
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from("course-files").getPublicUrl(fileName);
    ...
    try {
      ...
      if (activeTab === "video" && videoFile) {
        ...
        const contentPromises = attachmentFiles.map((file, index) =>
          supabase.from("module_content").insert({
            module_id: module.id,
            content_type: "file",
            title: file.name,
            file_url: file.url,
                ...
                <div className="space-y-4">
                  ...
                  {videoDuration && (
                    <p className="text-sm text-gray-600">
                      {t("module.videoDuration")}: {videoDuration}{" "}
                      {t("common.minutes")}
                    </p>
                  )}
                  <p className="text-xs text-gray-500">
